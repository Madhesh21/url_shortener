...
export default function Shortener() {
  ...
  const handleSubmit = (e) => {
    ...
    if (!text) {
      ...
      const shortenLink = async () => {
        const res = await fetch(`https://api.shrtco.de/v2/shorten?url=${text}`); //CHANGE THE URL OF THE API IT'S A DUMMY ONE.
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(links.full_short_link); //CHANGE THE full_short_link and original_link[ln:76] ACCORDING TO YOUR API.
